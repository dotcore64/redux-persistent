...
export default (selectors = {}, { storage = localStorage } = {}) =>
  ({ dispatch, getState }) =>
  (next) =>
  (action) => {
    if (action[HYDRATE]) {
      ...
      return Promise.all(
        Object.keys(selectors).map((key) =>
          Promise.resolve(storage.getItem(key)) // Normalize into promise
            .then((value) => {
    ...
    const prevValues = mapObject(
      ({ selector }) => selector(prevState),
      selectors,
    );
